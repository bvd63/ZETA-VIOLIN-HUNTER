...
- `scrapers/yahoo_auctions_jp.py` — NEW (2026-09-07 evening). Yahoo Auctions JP web search; runs only from non-EEA egress (Railway = Singapore, verified HTTP 200). Broad "エレキバイオリン" query first so the watchdog sees the site alive; auction links are `/jp/auction/{id}` on auctions.yahoo.co.jp; `data-auction-*` attributes preferred; 404 = no results. Parser verified against the real card structure reported in the Railway log (`Product`, `Product__titleLink`).

### Prompt 16 — full audit (2026-09-10, own review + independent Opus review, 43 findings)
Root causes of "something feels off" after 3 days of production, all fixed:
- **CRITICAL — noise/intent applied to descriptions** dropped real Zetas ("comes with a volume pedal", "nothing broken", "copy of the receipt", "cello" in the body). `filters.classify()` now decides on the TITLE first: when the title alone proves a Zeta violin, the body is checked only for homonym noise (Arc'teryx, replica, Zeta-Jones…) and "not for sale" phrases (WTB, case only, for parts…). Object-of-sale words (pedal, cover, cello, copy, sticker, FX, broken…) are title-only, or body-level only when the title is inconclusive.
- **CRITICAL — MIN/MAX_PRICE compared to local amounts**: ¥300,000 > 99999 → every Japanese listing dropped, Mercari/Yahoo could never alert. `_price_in_range` now converts to USD (`price_tracker.parse_price_usd`, live ECB rates).
- **CRITICAL — listings were marked seen before Telegram delivery**: `notifier.send_listings()` returns the delivered ones; `main._deliver()` marks seen / records the alert only for those, the rest stay unseen and retry next cycle.
- **HIGH** — liveness: generic markers ("has been sold", "no longer available", "sold out", "page not found") removed, buy/bid controls on the page = alive; the "redirected away" rule only fires when the path actually changed (was killing every `facebook.com/marketplace/item/…` hit); marker-based deaths go to `liveness_cache` (re-checked after 7 days), only 404/410 and Reverb "not live" are permanent; checks run concurrently (6) under a 180 s cap, in random order (no starvation), budget 60/scraper/cycle, deferred = left unseen.
- **HIGH** — `run_search_cycle` wrapped in `try/finally` (connections closed, stats + watchdog + digest always run); background tasks log exceptions via done-callbacks; scheduled job has `misfire_grace_time=3600` (default 1 s could silently drop the only run of the day); a scheduled run now WAITS for a running cycle instead of being skipped; `POST /search` answers 409 while busy.
- **HIGH** — startup cycle guarded: skipped when a cycle started < `STARTUP_RUN_GUARD_HOURS` (6) ago (deploys/crash loops no longer re-scrape and re-spend quotas — CLAUDE.md §7).
- **HIGH** — Craigslist: `IndexError` on short sapi items could abort the whole scraper (`len(it) < 5` guard, `return_exceptions=True`).
- **HIGH** — watchdog false alarms: guard-skipped / unconfigured cycles are recorded as `raw = -1` and neither count nor reset the zero streak (Google/Brave skipping after a deploy used to look like "0 results for 3 days"). Google/Brave guard back to 10 h so an evening deploy does not block the next 12:00 run; Google additionally enforces a per-Pacific-day query ledger (`GOOGLE_DAILY_QUOTA`, 96) so restarts can never exceed the free quota.
- **MEDIUM** — dedup only annotates ("♻️ Posibil același instrument ca …"), needs ≥ 3 discriminating tokens on both sides and both prices known within 8 %; price-drop alerts only for ids actually alerted; eBay `sort=newlyListed` + 2 pages (100 newest per query/marketplace), 16 supported marketplaces; `NEW_CONDITION_RX` word-bounded (Neuwertig / wie neu / come nuovo = used); `SOLD_RX` only leading/trailing/bracketed SOLD ("must be sold this week" passes); bass/guitar/viola in the title only matter when the title has no violin word; "sealed" removed from shop language; URL validity only rejects site roots and `/search`, `/category`… landing pages; WAL + busy_timeout on every SQLite connection; FX rates cached in memory; caption > 1000 chars goes as text instead of a truncated photo caption; Telegram update offset persisted (no command replay after restart); "no new listings" message now says how many sources answered / errored; `scraper.fetched` reset per attempt; Guitar Chimp products default to Used (used-only dealer); `_is_direct_host` matches exact domains (gumtree.com.au and mercari.com are verified, not dropped); Reddit sets `fetched` and uses `_make_id`; Mercari marks `skipped` when mercapi is missing; digest sent at the END of the digest-day cycle (was a separate job 30 min after start, racing the cycle); egress re-detected at cycle start if unknown.
- **Removed**: `ai_verifier.py` (dead), `tenacity`, `anthropic` deps. CI compiles every module and runs all 4 suites (94 filter cases).
- Known remaining (accepted): `/search`, `/status` unauthenticated unless `SEARCH_TOKEN` is set (no public domain today); DB writes are synchronous on the event loop (short); the 6 disabled scraper files are kept per §2.
...
- CONDITION (default "all" since 2026-09-10 = no condition filter, new dealer stock alerted too; "used" = second-hand only, drops New/Brand New/Open box/B-Stock + dealers + shop language)
...
- GOOGLE_DAILY_QUOTA (96 — hard per-Pacific-day ledger), GOOGLE_GUARD_HOURS / BRAVE_GUARD_HOURS (10)
- STARTUP_RUN_GUARD_HOURS (6 — no cycle at container start if one started more recently)
- MIN_PRICE / MAX_PRICE are in **USD** (local prices are converted first)
...
- Prompt 16 — Full audit (own + independent Opus review): title-first noise/intent, USD price range, mark-seen-after-delivery, liveness cache + concurrency, try/finally cycle, misfire grace, startup guard, Google quota ledger, dedup annotate-only, eBay newest+paging, WAL, Yahoo JP scraper, dead code removed ✅ COMPLETED (2026-09-10)
...
| 2026-09-10 | **Condition filter switched OFF by default** (`CONDITION=all`) | Owner, after seeing that the rule hid rare signature models sold new (JLP $6100, SV24 $3700): "elimină, să văd ce o să fie după". The `used` mode stays available via the env var. |
