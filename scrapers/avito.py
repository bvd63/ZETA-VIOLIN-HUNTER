...
class AvitoScraper(BaseScraper):
    ...
    RUSSIAN_CITIES = [
        "moscow", "stpetersburg", "novosibirsk", "yekaterinburg", "nizhnynovgorod",
        "kazan", "chelyabinsk", "omsk", "samara", "rostov",
    ]
    ...
    async def search(self) -> list:
        ...
        async with httpx.AsyncClient(timeout=15) as client:
            for city in self.RUSSIAN_CITIES:
                for kw in keywords:
                    try:
                        url = f"https://{city}.avito.ru/search"
                        params = {
                            "q": kw,
                        }
                        ...
                        for item in listings:
                            ...
                            unique_id = self._make_id("avito", item_url)
                        ...
                        log.warning(f"Avito {city} '{kw}' error: {e}")
